...
class DataSourceConfigManager:
    ...
    @classmethod
    def from_file(
        cls, config_file: str, auto_refresh: bool = False, refresh_interval: int = 60
    ) -> "DataSourceConfigManager":
        """
        ...
        """
        return cls(
            config_file=config_file, auto_refresh=auto_refresh, refresh_interval=refresh_interval
        )
    ...
    def _fetch_from_endpoint(self) -> int:
        ...
        for attempt in range(max_retries):
            try:
                ...
                response = httpx.get(
                    f"{self.endpoint}/api/data-sources", headers=headers, timeout=10.0
                )
    ...
    def get_all_sources(self, enabled_only: bool = True) -> Dict[str, Dict[str, Any]]:
        ...
        if enabled_only:
            return {
                sid: source for sid, source in self._sources.items() if source.get("enabled", True)
            }
